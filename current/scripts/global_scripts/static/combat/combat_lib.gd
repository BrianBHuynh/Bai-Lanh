...
static func phys_attack(card: Card, target: Card, damage: float) -> void:
	if is_instance_valid(target):
		var change: float = target.damage_physical(damage+card.phys_attack)
...
static func mag_attack(card: Card, target: Card, damage: float) -> void:
	if is_instance_valid(target):
		var change: float = target.damage_magical(damage+card.mag_attack)
		Combat.combat_board = Combat.combat_board + card.title + " dealt " + str(change) + " damage to " + target.title + " They have " + str(int(target.health)) + " health left! \n" 
		MoveLib.move_then_return(card, target.current_position)
		ShadersLib.apply_shader(card, target, ShadersLib.get_shader("Damage"))
...
static func multi_phys_attack(card: Card, target: Card, diceMax: int, times: int) -> void:
	if is_instance_valid(target):
		...
		for i: int in times:
			var change: float = target.damage_physical(Combat.RNG.randi_range(1,diceMax)+card.phys_attack)
...
static func poison(card:Card, target:Card, damage:float, duration:int) -> void:
	if is_instance_valid(target):
		...
		var status: StatusEffect = StatusEffect.new("poison", duration, "every_turn", damage, target)
		Status.apply(status)
		MoveLib.move_then_return(card, target.current_position)
		ShadersLib.apply_shader(card, target, ShadersLib.get_shader("Damage"))
...
static func lock_down(card:Card, target:Card) -> void:
	if is_instance_valid(target):
		...
		var status: StatusEffect = StatusEffect.new("slow", 2, "every_turn", target.speed, target)
		Status.apply(status)
		MoveLib.move_then_return(card, target.current_position)
		ShadersLib.apply_shader(card, target, ShadersLib.get_shader("Damage"))
...
static func phys_defense_up(_card:Card, target:Card) -> void:
	if is_instance_valid(target):
		...
		var status: StatusEffect = StatusEffect.new("phys_defense_up", 3, "on_turn", 3, target)
		Status.apply(status)
...
static func phys_attack_up(_card:Card, target:Card) -> void:
	if is_instance_valid(target):
		...
		var status: StatusEffect = StatusEffect.new("phys_attack_up", 3, "on_turn", 3, target)
		Status.apply(status)
		MoveLib.change_scale(target, Vector2(1.5,1.5))
